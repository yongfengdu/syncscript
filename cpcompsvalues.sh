...
function check_chart {
  ...
  for valuefile in $valuefiles; do
    ...
    if [[ -d $chart ]]; then
      echo "Copy $chart/$valuefile to GenAIExamples/$dest_dir"
      cp $chart/$valuefile ~/work/GenAIExamples/$dest_dir/
    else # common components
      echo "Copy $chart/$valuefile to GenAIComps/$dest_dir"
      cp common/$chart/$valuefile ~/work/GenAIComps/$dest_dir/
    fi
